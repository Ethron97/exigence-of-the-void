$data merge entity @s {Item:{components:{"minecraft:custom_name":[$(name_cost),{text:"- Skyslayer -",italic:false,bold:$(bold),color:"$(legendary)"}]},count:1,id:"minecraft:paper"}}
...
$data modify entity @s Item.components."minecraft:lore" append value [$(lore_cost),{text:": +2 minutes of ",bold:false,italic:false,color:"gray"},{text:"Jump Boost IV",bold:false,italic:false,color:"$(effect)"}]
