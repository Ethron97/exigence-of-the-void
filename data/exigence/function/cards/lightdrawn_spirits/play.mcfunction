...
execute if score #LastConsumeResult game.resources matches 1 as @e[type=minecraft:item,scores={game.entity.glow_remaining=1..}] at @s at @a[scores={dead=0},tag=ActivePlayer,sort=nearest,limit=1] run function exigence:cards/lightdrawn_spirits/private/pickup
