...
execute as @a[tag=ActivePlayer,limit=1,sort=arbitrary] run function exigence:profile/profile_node/story/grant {story:'talk_to_petitioner'}

# Trigger dialoge
execute at @s as @n[distance=..32,type=minecraft:villager,tag=NPC_Petitioner,tag=Game] run function exigence:npc/game/dialogue_petitioner
