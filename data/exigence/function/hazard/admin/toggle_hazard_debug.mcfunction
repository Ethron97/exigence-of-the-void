...
$execute if data storage exigence:debug {hazard:1} as @e[type=minecraft:marker,scores={node.property.object_level=$(levels)},tag=HazardNode] at @s run function exigence:hazard/node/debug/on
execute if data storage exigence:debug {hazard:0} as @e[type=minecraft:marker,tag=HazardNode] at @s run function exigence:hazard/node/debug/off
