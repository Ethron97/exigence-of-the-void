...
tag @e[type=minecraft:marker,scores={node.property.object_level=3},tag=MenaceNode] add CanTeleport
...
execute as @e[type=minecraft:enderman,tag=VoidMerchant] at @s as @e[type=minecraft:armor_stand,tag=CanTeleport,distance=..5] run tag @s remove CanTeleport
...
execute at @s run tag @e[type=minecraft:armor_stand,tag=CanTeleport,distance=..32,sort=nearest,limit=5] remove CanTeleport
