...
execute if score npc.petitioner game.dialogue matches 2 run say The flame... the flame returns!
execute if score npc.petitioner game.dialogue matches 3 run say Blessed one, may your days grow ever brighter!
execute if score npc.petitioner game.dialogue matches 4 run say Carry me away from this forsaken place, and my aid will be yours.
execute if score npc.petitioner game.dialogue matches 5 run tellraw @a[tag=ActivePlayer] [{text:"Interact with the Petitioner to carry them. ",color:"gray"},{text:"WARNING: You cannot put them down again once you pick them up.",color:"red"}]
execute if score npc.petitioner game.dialogue matches 6 run tag @s add Carried 
execute if score npc.petitioner game.dialogue matches 6 run tag @a[advancements={exigence:listener/interact/npc_petitioner_game=true}] add Carrying
