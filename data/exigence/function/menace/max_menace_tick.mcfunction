...
#execute if score ravager.aggro.cooldown tick_counter matches 1 at @a[tag=ActivePlayer,scores={dead=0}] at @e[type=minecraft:armor_stand,tag=MenaceNode,limit=1,sort=nearest] run function exigence:menace/eyeball/flyball
execute if score ravager.aggro.cooldown tick_counter matches 1 at @a[scores={dead=0},tag=ActivePlayer] positioned ~ ~5 ~ run function exigence:menace/eyeball/flyball
...
execute as @a[team=Enemy,tag=ActivePlayer] run function exigence:player/effects/break_invisibility
