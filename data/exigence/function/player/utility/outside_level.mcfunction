...
execute if score @s Temp matches 0 run scoreboard players add @s TimeOffLevel 1

# If on a level, reset time off level to 0
execute if score @s Temp matches 1 run scoreboard players set @s TimeOffLevel 0

# If time off level is > 10 seconds, give wither I for a second
execute if score SecondsCooldown TickCounter matches 3 if score @s TimeOffLevel matches 200.. run effect give @s wither 1 1 true
