...
$execute if score toggle.player debug matches 1 if score debug.level debug matches 3.. run say (D3 Player) Use item $(slot)
...
$execute if items entity @s weapon.$(slot) carrot_on_a_stick[custom_data~{item_name:'clock'}] run function exigence:player/use_item/clock/use
