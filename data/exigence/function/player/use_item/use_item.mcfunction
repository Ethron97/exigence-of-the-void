...
# DEBUG
execute if score toggle.player debug matches 1 if score debug.level debug matches 3.. run say (D3 Player) Use Item
...
execute unless items entity @s weapon.mainhand carrot_on_a_stick if items entity @s weapon.offhand carrot_on_a_stick run function exigence:player/use_item/use_item_hand {slot:'offhand'}
execute if items entity @s weapon.mainhand carrot_on_a_stick run function exigence:player/use_item/use_item_hand {slot:'mainhand'}
