# Call level subfunction for beastsense based on this player's active level

## CONSTRAINTS
#   AS/AT active player with dead=0, game.player.effects.beastsense=1..

#====================================================================================================

# Call bi-secondly to reduce lag
execute if score seconds.cooldown tick_counter matches 7 run function exigence:player/effects/beast_sense/private/beast_sense_level
execute if score seconds.cooldown tick_counter matches 17 run function exigence:player/effects/beast_sense/private/beast_sense_level

# If there are any ravagers left with the StartingSpeed boost, remove it so the player can't see them move fast via glow
#effect clear @e[type=minecraft:ravager,tag=StartingSpeed] speed
#tag @e[type=minecraft:ravager,tag=StartingSpeed] remove StartingSpeed
