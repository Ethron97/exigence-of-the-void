...
execute if score et.FromCard game.effect_temp matches 1 if score @s game.player.mod.detection matches 5 run scoreboard players add et.BonusTime game.effect_temp 600
...
execute if score et.FromCard game.effect_temp matches 1 if score #temp game.effect_temp matches 1.. run tellraw @s [{text:" └─ [",color:"gray"},{text:"Bonus Effects",color:"green"},{text:"]: +",color:"gray"},{score:{name:"#temp",objective:"game.effect_temp"},color:"gray"},{text:" seconds of ",color:"gray"},{text:"Clairvoyance",color:"yellow"}]
