...
execute as @s[scores={mod_LootBoots=1..}] run function exigence:player/modifiers/loot_boots

# If Circulation is active, generate one Treasure
execute if entity @s[scores={effect_circulation=1..}] run function exigence:treasure/queue/add_to_queue {source:"circulation",amount:1}
...
execute at @s[scores={effect_circulation=1..}] run playsound minecraft:block.note_block.bell neutral @s
