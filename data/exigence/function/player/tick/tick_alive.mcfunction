...
execute as @n[type=minecraft:item,tag=!SoulWarned,distance=..3,nbt={Item:{id:"minecraft:carrot_on_a_stick",components:{"minecraft:custom_data":{is_spellsling:"true"}}}}] run function exigence:player/dropped_soulbound
execute as @n[type=minecraft:item,tag=!SoulWarned,distance=..3,nbt={Item:{components:{"minecraft:custom_data":{is_soulbound:1b}}}}] run function exigence:player/dropped_soulbound
