...
execute if score toggle.game debug matches 1 if score debug.level debug matches 4.. run say (D4) Update waypoint ARDOR
...
execute if entity @s[tag=DroppedItem] run return run function exigence:game/other/waypoint/update/dropped_ardor
#----------------------------------------------------------------------------------------------------
...
#   If detection pinged:
execute if score @n[type=minecraft:item,tag=Ardor,distance=..1] game.waypoint.revealed_ticks matches 1.. run return run attribute @s waypoint_transmit_range base set 1000
