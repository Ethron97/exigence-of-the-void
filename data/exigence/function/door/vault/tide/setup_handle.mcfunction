...
summon minecraft:item_display -365.5 19.03125 -226.5 {Rotation:[0.0f,90.0f],CustomName:{text:"ItemDisplay | VaultHandleTide"},billboard:"fixed",Tags:["VaultHandle","NewVaultHandle"],\
...
scoreboard players operation @e[tag=NewVaultHandle] game.vault.handle.id = @e[type=minecraft:armor_stand,tag=VaultNode,tag=Vault_tide,limit=1] node.id
