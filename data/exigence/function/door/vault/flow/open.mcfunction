...
# Clear main door
fill -373 153 -27 -373 155 -25 air destroy

# Summon text displays
# Add local tag to the two corresponding altars to this vault
execute as @e[type=minecraft:armor_stand,tag=AltarNode,tag=Trial] if score @s VaultID = @e[type=minecraft:armor_stand,tag=VaultNode,tag=Vault_flow,limit=1] NodeID run tag @s add AltarTrialDisplay
...
execute as @e[type=minecraft:armor_stand,tag=AltarNode,tag=Trial,tag=AltarTrialDisplay,scores={NodeState=0}] at @s run function exigence:door/vault/flow/summon_trial_displays with entity @s
...
# Main display
summon minecraft:text_display -364.1 155.5 -25.5 {background:1,line_width:150,Rotation:[90.0f,0.0f],Tags:["VaultTrialDisplay","NewVaultTrialDisplay"],billboard:'fixed',text:[{text:"Light an altar to begin",color:"yellow"}]}
