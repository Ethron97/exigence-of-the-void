...
summon minecraft:item_display -373.03125 154.5 -25.5 {Rotation:[-90.0f,0.0f],CustomName:{text:"ItemDisplay | VaultHandleFlow"},billboard:"fixed",Tags:["VaultHandle","NewVaultHandle"],\
...
scoreboard players operation @e[tag=NewVaultHandle] game.vault.handle.id = @e[type=minecraft:armor_stand,tag=VaultNode,tag=Vault_flow,limit=1] node.id
