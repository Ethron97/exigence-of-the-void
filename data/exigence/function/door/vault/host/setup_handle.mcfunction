...
summon minecraft:item_display -372.5 3.50 -147.03125 {Rotation:[0.0f,0.0f],CustomName:{text:"ItemDisplay | VaultHandleHost"},billboard:"fixed",Tags:["VaultHandle","NewVaultHandle"],\
...
scoreboard players operation @e[tag=NewVaultHandle] game.vault.handle.id = @e[type=minecraft:armor_stand,tag=VaultNode,tag=Vault_host,limit=1] node.id
