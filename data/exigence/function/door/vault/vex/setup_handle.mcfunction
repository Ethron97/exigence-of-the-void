...
summon minecraft:item_display -393.5 77.03125 -252.5 {Rotation:[180.0f,90.0f],CustomName:{text:"ItemDisplay | VaultHandleVex"},billboard:"fixed",Tags:["VaultHandle","NewVaultHandle"],\
...
scoreboard players operation @e[tag=NewVaultHandle] game.vault.handle.id = @e[type=minecraft:armor_stand,tag=VaultNode,tag=Vault_vex,limit=1] node.id
