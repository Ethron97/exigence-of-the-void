...
execute if score toggle.vault debug matches 1 if score debug.level debug matches 4.. run say (D4) Setup handle (vex)

# Get nearby vault score
scoreboard players operation #compare node.id = @n[type=minecraft:marker,tag=VaultNode,tag=Vault_vex,distance=..5] node.id

# Summon new item display
summon minecraft:item_display ~ ~ ~ {Tags:["VaultHandle","NewVaultHandle","Vault_vex"],Rotation:[180.0f,90.0f],CustomName:{text:"ItemDisplay | VaultHandle-Vex",color:"gold"},billboard:"fixed",\
