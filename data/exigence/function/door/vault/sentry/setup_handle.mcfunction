...
summon minecraft:item_display -417.5 60.5 -206.96875 {Rotation:[180.0f,0.0f],CustomName:{text:"ItemDisplay | VaultHandleSentry"},billboard:"fixed",Tags:["VaultHandle","NewVaultHandle"],\
...
scoreboard players operation @e[tag=NewVaultHandle] game.vault.handle.id = @e[type=minecraft:armor_stand,tag=VaultNode,tag=Vault_sentry,limit=1] node.id
