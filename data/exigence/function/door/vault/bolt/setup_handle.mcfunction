...
summon minecraft:item_display -395.96875 154.5 -17.5 {Rotation:[90.0f,0.0f],CustomName:{text:"ItemDisplay | VaultHandleBolt"},billboard:"fixed",Tags:["VaultHandle","NewVaultHandle"],\
...
scoreboard players operation @e[tag=NewVaultHandle] game.vault.handle.id = @e[type=minecraft:armor_stand,tag=VaultNode,tag=Vault_bolt,limit=1] node.id
