...
# DEBUG
execute if score toggle.vault debug matches 1 if score debug.level debug matches 3.. run say (D3) Open vault (player)
...
execute if score @s game.player.active_level matches 3 run tellraw @s [{text:"[!] ",color:"yellow"},{text:"Light an altar to decide your fate",color:"gray"}]
