...
execute if score toggle.vault debug matches 1 if score debug.level debug matches 4.. run say (D4) Setup handle (wild)

# Get nearby vault score
scoreboard players operation #compare node.id = @n[type=minecraft:marker,tag=VaultNode,tag=Vault_wild,distance=..5] node.id

# Summon new item display
summon minecraft:item_display ~ ~ ~ {Tags:["VaultHandle","NewVaultHandle","Vault_wild"],Rotation:[-180.0f,0.0f],CustomName:{text:"ItemDisplay | VaultHandle-Wild",color:"gold"},billboard:"fixed",\
