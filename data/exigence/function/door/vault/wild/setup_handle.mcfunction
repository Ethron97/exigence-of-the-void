...
summon minecraft:item_display -403.5 65.5 -235.96875 {Rotation:[-180.0f,0.0f],CustomName:{text:"ItemDisplay | VaultHandleWild"},billboard:"fixed",Tags:["VaultHandle","NewVaultHandle"],\
...
scoreboard players operation @e[tag=NewVaultHandle] game.vault.handle.id = @e[type=minecraft:armor_stand,tag=VaultNode,tag=Vault_wild,limit=1] node.id
