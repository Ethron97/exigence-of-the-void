...
# Kill item displays
$execute as @e[type=minecraft:item_display,tag=VaultHandle] if score @s game.vault.handle.id = #compare game.vault.handle.id at @s run particle minecraft:item{item:{id:"minecraft:$(vault)_armor_trim_smithing_template"}} ^ ^ ^-0.2 0.3 0.3 0.3 0.1 20
$execute as @e[type=minecraft:item_display,tag=VaultHandleKey] if score @s game.vault.handle.id = #compare game.vault.handle.id at @s run particle minecraft:item{item:{id:"minecraft:$(vault)_armor_trim_smithing_template"}} ^ ^ ^-0.2 0.3 0.3 0.3 0.1 20
execute as @e[type=minecraft:item_display,tag=VaultHandle] if score @s game.vault.handle.id = #compare game.vault.handle.id run kill @s
execute as @e[type=minecraft:item_display,tag=VaultHandleKey] if score @s game.vault.handle.id = #compare game.vault.handle.id run kill @s
