...
summon minecraft:item_display -373.03125 154.5 6.5 {Rotation:[-90.0f,0.0f],CustomName:{text:"ItemDisplay | VaultHandleEye"},billboard:"fixed",Tags:["VaultHandle","NewVaultHandle"],\
...
scoreboard players operation @e[tag=NewVaultHandle] game.vault.handle.id = @e[type=minecraft:armor_stand,tag=VaultNode,tag=Vault_eye,limit=1] node.id
