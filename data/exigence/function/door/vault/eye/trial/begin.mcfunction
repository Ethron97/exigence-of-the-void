...
# DEBUG
execute if score toggle.trial debug matches 1 if score debug.level debug matches 3.. run say (D3) Begin Eye
...
execute if entity @s[tag=Trial] as @a unless score @s game.player.vault_code matches 4 run title @s subtitle [{"selector":"@a[scores={game.player.vault_code=4}]",color:"yellow"},\
{text:" entered the ",color:"gray"},{text:"Trial of Sight",color:"aqua"}]
execute if entity @s[tag=Crucible] as @a unless score @s game.player.vault_code matches 4 run title @s subtitle [{"selector":"@a[scores={game.player.vault_code=4}]",color:"yellow"},\
{text:" entered the ",color:"gray"},{text:"Crucible of Sight",color:"light_purple"}]
execute as @a unless score @s game.player.vault_code matches 4 run title @s title ""
...
execute if score game.is_active game.state matches 1 run scoreboard players add @s[tag=Trial] profile.data.vaults.cr.eye_trial_try 1
execute if score game.is_active game.state matches 1 run scoreboard players add @s[tag=Crucible] profile.data.vaults.cr.eye_crucible_try 1
