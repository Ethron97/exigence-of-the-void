...
summon minecraft:item_display -395.96875 154.5 14.5 {Rotation:[90.0f,0.0f],CustomName:{text:"ItemDisplay | VaultHandleSilence"},billboard:"fixed",Tags:["VaultHandle","NewVaultHandle"],\
...
scoreboard players operation @e[tag=NewVaultHandle] game.vault.handle.id = @e[type=minecraft:armor_stand,tag=VaultNode,tag=Vault_silence,limit=1] node.id
