...
# Clear crucible pots
fill -446 153 18 -444 153 10 air replace minecraft:decorated_pot

# Check if any players are entering with this vault key
scoreboard players set #temp Temp 0
execute store result score #temp Temp run clear @a[tag=Predungeon] gold_ingot[custom_data~{vault:'silence'}] 0
