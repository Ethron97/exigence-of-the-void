...
$execute if score Difficulty DungeonRun matches 1.. run tag @e[type=minecraft:armor_stand,tag=EchoNode,scores={EchoDifficulty=$(min_difficulty)..$(max_difficulty),ObjectLevel=$(level)},sort=random,limit=1] add ChosenEchoNode
