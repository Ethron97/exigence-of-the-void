...
execute if score toggle.menu debug matches 1 if score debug.level debug matches 3.. run say (D3 Menu) Call sub-refresh based on button tags
...
execute if entity @s[tag=EmberShopAnalyzerDisplay] run return run function exigence:hub/deck_analyzer/refresh_button/calls/refresh/ember_shop with entity @s item.components."minecraft:custom_data"
...
execute if score toggle.menu debug matches 1 if score debug.level debug matches 3.. run say (D3 Menu) No valid sub-refresh found
