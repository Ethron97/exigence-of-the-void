...
$execute if score toggle.menu debug matches 1 if score debug.level debug matches 4.. run say (D4 Menu) Hover call $(preview_hover_scale)
...
# Glow preview
$execute if entity @s[tag=HasPreview] at @s as @n[type=minecraft:text_display,scores={IDID=$(idid)},tag=SettingPreview,distance=..10] run \
