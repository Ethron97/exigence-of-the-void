...
# Get value player
scoreboard players operation @n[type=item_display,tag=NewItemDisplay,distance=..0.1] Random = @p[tag=ProfileSelecting,distance=..16] career.settings.show_resource_underflow

# Set type to Toggle
tag @n[type=item_display,tag=NewItemDisplay,distance=..0.1] add Toggle

# Set data
data modify entity @n[type=item_display,tag=NewItemDisplay,distance=..0.1] item.components."minecraft:custom_data".scoreboard set value "show_resource_underflow"
...
data modify entity @n[type=item_display,tag=NewItemDisplay,distance=..0.1] item.components."minecraft:custom_data".display set value {text:"Notify resource missing"}
data modify entity @n[type=item_display,tag=NewItemDisplay,distance=..0.1] item.components."minecraft:custom_data".display0 set value {text:"Notify resource missing"}
data modify entity @n[type=item_display,tag=NewItemDisplay,distance=..0.1] item.components."minecraft:custom_data".display1 set value {text:"Notify resource missing"}
...
# Summon preview
execute at @s positioned ~5.47 ~1.85 ~0.12 as @n[type=item_display,tag=NewItemDisplay,distance=..16] run function exigence:hub/profile_selector/menu/display/settings/calls/private/create_preview with entity @s item.components."minecraft:custom_data"
