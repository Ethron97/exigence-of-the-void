...
# Get value player
scoreboard players operation @n[distance=..0.1,type=item_display,tag=NewItemDisplay] Random = @p[tag=ProfileSelecting,distance=..16] career.settings.show_resource_consume
...
data modify entity @n[distance=..0.1,type=item_display,tag=NewItemDisplay] item.components."minecraft:custom_data".display set value {text:"Notify resource consumed"}
