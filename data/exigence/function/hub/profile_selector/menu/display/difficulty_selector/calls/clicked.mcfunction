...
execute if score toggle.menu debug matches 1 if score debug.level debug matches 3.. run say (D3) Clicked difficulty selecotr
...
# Copy picked item
#$data modify entity @n[distance=..5,type=minecraft:item_display,tag=SlotDisplay,scores={IDID=$(parent_idid)}] item.id set from entity @s item.id
# TODO also the custom model data?

# Mark the title of this one to be kept around
$execute at @s as @n[type=minecraft:text_display,scores={IDID=$(idid)},tag=DifficultyTitle,distance=..5] run tag @s add KeepDisplay
$execute at @s as @n[type=minecraft:text_display,scores={IDID=$(idid)},tag=DifficultyTitle,distance=..5] run data modify entity @s text set value $(reminder)
$execute at @s as @n[type=minecraft:text_display,scores={IDID=$(idid)},tag=DifficultyTitle,distance=..5] \
...
run tp @n[type=minecraft:text_display,scores={IDID=$(idid)},tag=DifficultyTitle,distance=..5] ~ ~0.72 ~
