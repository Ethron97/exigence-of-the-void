...
execute at @s run playsound minecraft:block.beacon.activate block @p[distance=..24] ~ ~ ~ 1.0 1.2
...
$execute at @s as @p[distance=..24,scores={hub.player.profile_selector_id=$(profile_selector_id)}] in exigence:profile_data run function exigence:profile/profile_node/new_profile
