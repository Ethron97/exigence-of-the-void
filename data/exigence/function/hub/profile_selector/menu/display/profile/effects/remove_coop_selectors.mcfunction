...
$execute at @s as @e[distance=..24,type=minecraft:item_display,tag=CoopSelectorDisplay,nbt={item:{components:{"minecraft:custom_data":{slot_id:"$(slot_id)"}}}}] \
...
# Remove display
$execute at @s run kill @e[distance=..24,type=minecraft:item_display,tag=CoopSelectorDisplay,nbt={item:{components:{"minecraft:custom_data":{slot_id:"$(slot_id)"}}}}]
