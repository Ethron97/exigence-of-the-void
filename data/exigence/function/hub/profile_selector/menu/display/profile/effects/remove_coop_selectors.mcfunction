...
execute if score toggle.menu debug matches 1 if score debug.level debug matches 3.. run say (D3) Remove coop selectors
...
# Remove display
$execute at @s run kill @e[type=minecraft:item_display,tag=CoopSelectorDisplay,distance=..16,nbt={item:{components:{"minecraft:custom_data":{slot_id:"$(slot_id)"}}}}]
