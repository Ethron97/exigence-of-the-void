...
execute if entity @s[tag=Joined] run tellraw @s [{text:"✖ You cannot leave the Profile Selector room while awaiting a co-op profile! Use ",color:"red"}\
,{text:"/trigger DeclineCoopInvite",color:gray,underlined:true,click_event:{action:"run_command",command:"trigger DeclineCoopInvite"}\
,hover_event:{action:"show_text",value:{text:"Click to run command"}}}\
,{text:" to leave the profile.",color:red,underlined:false}]
