...
execute if score toggle.hub debug matches 1 if score debug.level debug matches 3.. run say (D3) Unload room

# Get loaded profile
scoreboard players reset #compare profile.node.profile_id
execute at @s as @n[type=minecraft:item_display,tag=ProfileLoaded,tag=Selected,distance=..16] store result score #compare profile.node.profile_id run data get entity @s item.components."minecraft:custom_data".profile_id
