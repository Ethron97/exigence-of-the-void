...
execute if score toggle.hub debug matches 1 if score debug.level debug matches 3.. run say (D3 Hub) Load deck analyzer [ember shop]
...
# Item Displays
tag @e[type=minecraft:item_display,tag=NewItemDisplay,distance=..8] add EmberShopAnalyzerDisplay
tag @e[type=minecraft:item_display,tag=NewItemDisplay,distance=..6] remove NewItemDisplay

# Edit text displays
tag @e[type=minecraft:text_display,tag=NewTextDisplay,distance=..8] add EmberShopAnalyzerDisplay
tag @e[type=minecraft:text_display,tag=NewTextDisplay,distance=..8] remove NewTextDisplay

# Edit block displays
tag @e[type=minecraft:block_display,tag=NewBlockDisplay,distance=..8] add EmberShopAnalyzerDisplay
