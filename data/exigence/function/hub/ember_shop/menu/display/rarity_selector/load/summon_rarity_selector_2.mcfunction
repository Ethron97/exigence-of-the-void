...
execute if score toggle.menu debug matches 1 if score debug.level debug matches 4.. run say (D4 Menu) Summon rarity selector (uncommon)
...
,item:{id:"minecraft:paper",count:1,components:{"minecraft:custom_model_data":{strings:["card_uncommon"]}\
,"minecraft:custom_name":[{text:"> ",color:"gray"},{text:"Uncommon",color:"green"},{text:" <",color:"gray"}]\
,"minecraft:lore":[[{text:"Uncommon",color:"green"}],[{text:"Refresh\n",color:"gray"},{text:"Uncommon",color:"green"}]]\
...
# Set tag
tag @n[type=minecraft:item_display,tag=NewItemDisplay,distance=..0.1] add Uncommon

scoreboard players set @n[type=minecraft:item_display,tag=NewItemDisplay,distance=..0.1] ember_shop.rarity 2
