...
execute if score toggle.menu debug matches 1 if score debug.level debug matches 4.. run say (D4 Menu) Summon rarity selector (common)
...
,item:{id:"minecraft:paper",count:1,components:{"minecraft:custom_model_data":{strings:["card_common"]}\
,"minecraft:custom_name":[{text:"> ",color:"gray"},{text:"Common",color:"dark_aqua"},{text:" <",color:"gray"}]\
,"minecraft:lore":[[{text:"Common",color:"dark_aqua"}],[{text:"Refresh\n",color:"gray"},{text:"Common",color:"dark_aqua"}]]\
...
# Set tag
tag @n[type=minecraft:item_display,tag=NewItemDisplay,distance=..0.1] add Common

scoreboard players set @n[type=minecraft:item_display,tag=NewItemDisplay,distance=..0.1] ember_shop.rarity 1
