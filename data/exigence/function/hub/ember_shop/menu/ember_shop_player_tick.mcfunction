...
execute if score toggle.hub debug matches 1 if score debug.level debug matches 5.. run say (D5) Ember Shop tick player

scoreboard players operation #compare career.player_id = @s career.player_id

# Mark interaction
execute as @e[type=interaction,tag=EmberShopInteraction,distance=..8] \
...
function exigence:hub/ember_shop/menu/display/get_looking
