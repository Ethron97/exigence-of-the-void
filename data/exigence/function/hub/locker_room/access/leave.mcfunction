...
execute if score toggle.hub debug matches 1 if score debug.level debug matches 3.. run say (D3 Hub) Leaving Locker Room
...
execute as @n[type=minecraft:marker,tag=LockerRoomNode,distance=..0.1] at @s run function exigence:hub/locker_room/node/unload_room
