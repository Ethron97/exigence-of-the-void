...
execute if score #player_found Temp matches 1 in exigence:profile_data positioned 8 3 8 as @e[distance=..20,type=armor_stand,tag=PlayerNode] \
...
execute if score #player_found Temp matches 0 in exigence:profile_data positioned 8 3 8 as @e[distance=..20,type=armor_stand,tag=PlayerNode] \
if score @s profile.node.player_id = #compare career.player_id run tellraw @a[tag=Predungeon] \
