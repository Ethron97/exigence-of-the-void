...
# BEAT LEVEL:
execute if entity @p[distance=..16,tag=Predungeon,advancements={exigence:story/win_difficulty_4=true}] run tag @s add GoodUnlock
# TODO attempted level profile.data.winless.attempted_D4

#   Once you've at least ATTEMPTED (TODO) leve1 1, the level icon is known.
data modify entity @s[tag=GoodUnlock] item.components."minecraft:custom_model_data".strings set value ["level_4"]
