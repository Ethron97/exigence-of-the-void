...
# ATTEMPTED LEVEL:
execute if score #reached_2 Temp matches 1.. run tag @s add GoodKnown
...
execute at @s[tag=GoodUnlock,tag=GoodKnown,tag=!GoodCards] run function exigence:hub/predungeon/menu/display/level_chooser/load/2/model_locked
execute at @s[tag=GoodUnlock,tag=GoodKnown,tag=GoodCards] run function exigence:hub/predungeon/menu/display/level_chooser/load/2/model_available
...
# Set name (lore 0)
data modify entity @s[tag=GoodKnown] item.components."minecraft:lore"[0] set value [{text:"Mirror Mines",color:"#c7c1c1"}]

# Set locked reason (lore 1)
data modify entity @s[tag=!GoodUnlock] item.components."minecraft:lore"[1] set value [{text:"Rescue Supplier!",color:"#cf4f4f"}]
...
execute if entity @s[tag=GoodUnlock] at @n[type=minecraft:marker,tag=DoorSlotMarker2,distance=..5] positioned ~-0.15625 ~ ~-0.0525 \
run function exigence:hub/predungeon/menu/display/level_chooser/load/2/summon_deck_size