...
execute if score toggle.menu debug matches 1 if score debug.level debug matches 3.. run say (D3 Menu) clicked pre button
...
execute as @n[type=minecraft:marker,tag=PredungeonMenuNode,distance=..5] at @s run function exigence:hub/predungeon/validate/validate
...
# Close door slot
execute at @s as @n[type=minecraft:item_display,tag=DoorSlotFill0,tag=Open,distance=..5] run function exigence:hub/predungeon/menu/display/warp_door/slot/close_slot

# Open level chooser menu (schedule)
#execute at @s at @n[distance=..5,type=minecraft:marker,tag=PredungeonMenuNode] run
