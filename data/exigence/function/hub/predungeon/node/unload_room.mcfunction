...
# DEBUG
execute if score toggle.hub debug matches 1 if score debug.level debug matches 3.. run say (D3 Hub) Unload predungeon
...
kill @e[type=minecraft:marker,tag=PredungeonAnalyzerDisplay,distance=..10]

# Remove interactions
function exigence:hub/predungeon/node/kill_all_interactions

# Unload predungeon menu
execute positioned ~ ~ ~5 run kill @n[type=minecraft:marker,tag=PredungeonMenuNode,distance=..5]

execute positioned ~ ~ ~5 run kill @e[type=#exigence:display,tag=PredungeonDisplay,distance=..5]
execute positioned ~ ~ ~5 run kill @e[type=minecraft:marker,tag=PredungeonDisplay,distance=..5]

# Clean up the warp stuff
kill @e[type=minecraft:block_display,tag=schem_display,distance=..150]
