...
execute if score toggle.hub debug matches 1 if score debug.level debug matches 3.. run say (D3 Hub) Try enter PRE DUNGEON
...
# Shop is in use
execute if entity @e[x=100,y=199,z=100,dx=0,dy=1,dz=0,type=minecraft:marker,scores={hub.room.room_type=2},tag=RoomNode] at @s run return run function exigence:hub/predungeon/access/fail/in_use
#----------------------------------------------------------------------------------------------------

# Game is active
execute if entity @e[x=100,y=199,z=100,dx=0,dy=1,dz=0,type=minecraft:marker,scores={hub.room.room_type=3},tag=RoomNode] at @s run return run function exigence:hub/predungeon/access/fail/in_use_game
