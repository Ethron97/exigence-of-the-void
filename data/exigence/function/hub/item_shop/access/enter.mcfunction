...
run scoreboard players operation @n[x=100,y=199,z=100,dx=0,dy=1,dz=0,type=marker,scores={hub.room.room_type=9},tag=RoomNode] hub.entity.profile_id = @s profile.player.profile_id
execute if score @s profile.player.coop_profile_id matches 1.. in exigence:hub \
run scoreboard players operation @n[x=100,y=199,z=100,dx=0,dy=1,dz=0,type=marker,scores={hub.room.room_type=9},tag=RoomNode] hub.entity.coop_profile_id = @s profile.player.coop_profile_id
#   PLAYER/NODE
