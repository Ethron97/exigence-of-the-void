...
run scoreboard players operation @n[distance=..1,type=marker,tag=RoomNode,scores={hub.room.room_type=9}] hub.entity.profile_id = @s profile.player.profile_id
execute if score @s profile.player.coop_profile_id matches 1.. in exigence:hub positioned 0 153 0 \
run scoreboard players operation @n[distance=..1,type=marker,tag=RoomNode,scores={hub.room.room_type=9}] hub.entity.coop_profile_id = @s profile.player.coop_profile_id
#   PLAYER/NODE
