...
$execute as @n[type=minecraft:item,tag=NSS,distance=..1] run function exigence:hub/item_shop/item/$(item_name)/summon with storage exigence:colors
...
#execute as @n[type=minecraft:item,tag=NSS,distance=..1] if items entity @s container.* #exigence:armor \
#run data modify entity @s Item.components."minecraft:tooltip_display".hidden_components set value ["attribute_modifiers","trim","enchantments","dyed_color"]
