...
execute as @s[tag=!Hover,tag=DisplayingCost] unless entity @a[scores={shop.player.holding_currency=1},tag=ItemShop,distance=..8] run function exigence:menu/item_display/item_shop_display/cost_display/delete with entity @s item.components."minecraft:custom_data"

# If no player is within ..8, early return
execute unless entity @a[tag=ItemShop,distance=..8] run return 0
#----------------------------------------------------------------------------------------------------

# If holding currency: Add displays that aren't already displaying
execute as @s[tag=!DisplayingCost,tag=!Hover] if entity @a[scores={shop.player.holding_currency=1},tag=ItemShop,distance=..8] run function exigence:menu/money_menus/handle_cost_display
