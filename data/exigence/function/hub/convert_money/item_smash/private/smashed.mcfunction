...
execute at @e[type=minecraft:item,tag=ConvertMoneyCoinItem,tag=CurrentBatch,sort=random,limit=1] run playsound minecraft:block.amethyst_block.break neutral @a ~ ~ ~ 1 1
execute at @e[type=minecraft:item,tag=ConvertMoneyCoinItem,tag=CurrentBatch,sort=random,limit=1] run playsound minecraft:block.note_block.chime neutral @a ~ ~ ~ 1 1
