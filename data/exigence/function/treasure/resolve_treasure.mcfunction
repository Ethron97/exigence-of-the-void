...
execute if score #LevelKeyCheck Random matches 1 unless score 1.keys_dropped game.level_doors matches 5.. unless score 1.opened game.level_doors matches 1 if entity @a[scores={game.player.active_level=1},tag=ActivePlayer] as @e[type=minecraft:armor_stand,scores={ObjectLevel=1},tag=TreasureNode,tag=!L0,tag=!KeyDropper,tag=!KeyBlacklist,sort=random,limit=1] run function exigence:treasure/keys/private/drop_key_1
execute if score #LevelKeyCheck Random matches 1 unless score 2.keys_dropped game.level_doors matches 5.. unless score 2.opened game.level_doors matches 1 if entity @a[scores={game.player.active_level=2},tag=ActivePlayer] as @e[type=minecraft:armor_stand,scores={ObjectLevel=2},tag=TreasureNode,tag=!L0,tag=!KeyDropper,tag=!KeyBlacklist,sort=random,limit=1] run function exigence:treasure/keys/private/drop_key_2
execute if score #LevelKeyCheck Random matches 1 unless score 3.keys_dropped game.level_doors matches 5.. unless score 3.opened game.level_doors matches 1 if entity @a[scores={game.player.active_level=3},tag=ActivePlayer] as @e[type=minecraft:armor_stand,scores={ObjectLevel=3},tag=TreasureNode,tag=!L0,tag=!KeyDropper,tag=!KeyBlacklist,sort=random,limit=1] run function exigence:treasure/keys/private/drop_key_3


## WARD CRYSTALS
#   If player is on levels 3 or 4, and at least one crystal exists
execute if score Difficulty DungeonRun matches 4 if entity @e[type=interaction,tag=Crystal] if entity @a[scores={dead=0,game.player.active_level=3..4},tag=ActivePlayer] run function exigence:game/other/wards/proc_resonance
