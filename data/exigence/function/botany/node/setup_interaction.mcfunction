...
execute as @e[x=-520,y=-64,z=-287,dx=345,dy=345,dz=345,type=minecraft:interaction,tag=BerryInteract] if score @s game.interaction.node.id = #compare node.id run kill @s
...
# Assign id
execute at @s run scoreboard players operation @e[type=minecraft:interaction,tag=NewBerryInteract,distance=..3] game.interaction.node.id = #compare node.id
