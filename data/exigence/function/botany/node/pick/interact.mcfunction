...
# Pick berry bush
execute at @s[nbt={SelectedItem:{id:"minecraft:golden_hoe"}}] run execute as @e[distance=..10,type=minecraft:armor_stand,tag=BerryNode] if score @s NodeID = #compare NodeID run function exigence:botany/node/pick/pick_bush_harvest_b
execute at @s[nbt=!{SelectedItem:{id:"minecraft:golden_hoe"}}] run execute as @e[distance=..10,type=minecraft:armor_stand,tag=BerryNode] if score @s NodeID = #compare NodeID run function exigence:botany/node/pick/pick_bush
