...
$scoreboard players add @s[tag=BerryNode] node.berry.max_berries $(max)

# Set to age 3
execute at @s[tag=BerryNode] run setblock ~ ~ ~ minecraft:sweet_berry_bush[age=3]

# If min > 1 or max > 3, add particles
execute as @s[tag=BerryNode] if score @s node.berry.min_berries matches 1.. run tag @s add Overgrown
execute as @s[tag=BerryNode] if score @s node.berry.max_berries matches 3.. run tag @s add Overgrown
