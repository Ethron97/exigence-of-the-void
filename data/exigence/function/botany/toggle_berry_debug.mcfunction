...
$execute if data storage exigence:debug {berry:1} as @e[type=minecraft:armor_stand,tag=BerryNode,scores={ObjectLevel=$(levels)}] run data merge entity @s {Glowing:1b,CustomNameVisible:1b,Invisible:0b,Marker:0b}
execute if data storage exigence:debug {berry:0} as @e[type=minecraft:armor_stand,tag=BerryNode] run data merge entity @s {Glowing:0b,CustomNameVisible:0b,Invisible:1b,Marker:1b}
