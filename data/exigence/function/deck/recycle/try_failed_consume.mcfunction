...
execute positioned -302.42 2.00 -104.47 as @e[distance=..24,type=minecraft:armor_stand,tag=Card,scores={Deck=1..},tag=Played,tag=!Instant,tag=!Recycled,tag=!Recycler,tag=!Spellbound,tag=FailedConsume,limit=1,sort=random] run function exigence:cards/recycle
