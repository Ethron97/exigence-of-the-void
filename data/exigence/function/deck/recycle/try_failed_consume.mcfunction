...
execute positioned -302.42 2.00 -104.47 as @e[type=minecraft:armor_stand,scores={game.deck=1..},tag=Card,tag=Played,tag=!Instant,tag=!Recycled,tag=!Recycler,tag=!Spellbound,tag=FailedConsume,distance=..24,sort=random,limit=1] run function exigence:cards/recycle
