...
execute at @a[tag=ItemShop,scores={shop.player.holding_currency=1}] as @e[distance=..8,type=minecraft:item_display,tag=ItemShopDisplay,tag=!DisplayingCost,tag=!Hover] run function exigence:menu/money_menus/handle_cost_display

# Delete if a displayer (holding currency) ever gets further than 8 blocks away
execute as @e[distance=..16,type=minecraft:item_display,tag=ItemShopDisplay,tag=DisplayingCost,tag=!Hover] at @s unless entity @a[tag=ItemShop,distance=..8,scores={shop.player.holding_currency=1}] run function exigence:menu/item_display/item_shop_display/cost_display/delete with entity @s item.components."minecraft:custom_data"
...
scoreboard players remove @e[distance=..16,type=minecraft:item_display,tag=ItemShopDisplay,scores={shop.frame.purchase_cooldown=1..}] shop.frame.purchase_cooldown 1
