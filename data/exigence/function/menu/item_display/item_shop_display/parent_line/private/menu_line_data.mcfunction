...
execute as @n[type=minecraft:block_display,tag=NewMenuLine,distance=..1] at @s anchored eyes facing entity @n[tag=DrawLineTo,distance=..10] eyes run tp @s ~ ~ ~ ~ ~
...
execute at @s run data modify entity @s Rotation set from entity @n[type=minecraft:marker,tag=MenuAnchor,distance=..10] Rotation
