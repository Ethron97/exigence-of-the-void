...
execute as @s[tag=!IsParentResearched] run data modify entity @s item.components."minecraft:custom_data".locked_reason set value {text:"Research previous item!",color: "red"}
...
execute as @s[tag=!IsTierUnlocked,scores={shop.frame.tier=7..8}] run data modify entity @s item.components."minecraft:custom_data".locked_reason set value {text:"Tier locked: Return from Level 4!",color: "red"}
