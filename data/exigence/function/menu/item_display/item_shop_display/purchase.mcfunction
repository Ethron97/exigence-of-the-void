...
$execute at @s[tag=CanBuy,tag=!OneTime,tag=Artifact] if score @p[distance=..16,tag=ItemShop,tag=Interacting] shop.player.money_to_spend matches $(money_cost).. run function exigence:menu/item_display/item_shop_display/purchase/money_artifact with entity @s item.components."minecraft:custom_data"
$execute at @s[tag=CanResearch,tag=!Artifact] if score @p[distance=..16,tag=ItemShop,tag=Interacting] shop.player.research_to_spend matches $(research_cost).. run function exigence:menu/item_display/item_shop_display/purchase/research with entity @s item.components."minecraft:custom_data"
