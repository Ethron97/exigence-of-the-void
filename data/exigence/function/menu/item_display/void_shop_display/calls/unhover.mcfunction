...
# Undisplay costs
$execute unless entity @a[tag=ActivePlayer,scores={dead=0,shop.player.void_shopping_id=$(VoidShopID),shop.player.holding_currency=1}] as @s[tag=VoidShopDisplay,tag=DisplayingCost] run function exigence:menu/item_display/void_shop_display/cost_display/delete with entity @s item.components."minecraft:custom_data"
