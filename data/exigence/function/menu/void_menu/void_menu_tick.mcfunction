...
execute as @a[scores={shop.player.looking_at_idid=0},tag=VoidShopping] run function exigence:player/utility/interaction/tp_home
...
execute as @e[type=minecraft:enderman,tag=VoidMerchant] at @s if entity @e[type=#exigence:scares_merchant,distance=..10] run function exigence:npc/game/void_merchant/scare
...
execute as @e[type=minecraft:enderman,tag=VoidMerchant,tag=Loaded] at @s unless entity @p[scores={dead=0},tag=ActivePlayer,tag=VoidShopping,distance=..6] run function exigence:menu/void_menu/unload
...
scoreboard players remove @e[type=minecraft:item_display,scores={shop.frame.purchase_cooldown=1..},tag=VoidShopDisplay] shop.frame.purchase_cooldown 1
