...
execute if entity @a[tag=VoidShopping,scores={shop.player.currency_amounts_changed=1}] run function exigence:menu/void_menu/update_purchasability
...
execute at @a[tag=VoidShopping,scores={shop.player.holding_currency=1}] as @e[distance=..8,type=minecraft:item_display,tag=VoidShopDisplay,tag=!DisplayingCost,tag=!Hover] run tag @s add CreateCostDisplay
...
execute as @e[type=minecraft:item_display,tag=VoidShopDisplay,tag=DisplayingCost,tag=!Hover] at @s unless entity @a[tag=VoidShopping,distance=..8,scores={shop.player.holding_currency=1}] run function exigence:menu/item_display/void_shop_display/cost_display/delete with entity @s item.components."minecraft:custom_data"
