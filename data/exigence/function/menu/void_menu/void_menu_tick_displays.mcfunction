...
execute if entity @a[scores={shop.player.currency_amounts_changed=1},tag=VoidShopping] run function exigence:menu/void_menu/update_purchasability
...
execute at @a[scores={shop.player.holding_currency=1},tag=VoidShopping] as @e[type=minecraft:item_display,tag=VoidShopDisplay,tag=!DisplayingCost,tag=!Hover,distance=..8] run tag @s add CreateCostDisplay
...
execute as @e[type=minecraft:item_display,tag=VoidShopDisplay,tag=DisplayingCost,tag=!Hover] at @s unless entity @a[scores={shop.player.holding_currency=1},tag=VoidShopping,distance=..8] run function exigence:menu/item_display/void_shop_display/cost_display/delete with entity @s item.components."minecraft:custom_data"
