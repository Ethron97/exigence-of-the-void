...
execute if score resource.green_consume deck.analysis matches 0 if score resource.red_consume deck.analysis matches 0 if score resource.aqua_consume deck.analysis matches 0 run \
...
execute if score resource.green_consume deck.analysis matches 1.. run bossbar set exigence:resources players @a[scores={game.player.vault_code=0,career.settings.resource_bossbar_style=1..}]
execute if score resource.red_consume deck.analysis matches 1.. run bossbar set exigence:resources players @a[scores={game.player.vault_code=0,career.settings.resource_bossbar_style=1..}]
execute if score resource.aqua_consume deck.analysis matches 1.. run bossbar set exigence:resources players @a[scores={game.player.vault_code=0,career.settings.resource_bossbar_style=1..}]
