...
execute unless entity @e[type=minecraft:armor_stand,tag=VarianceNode,tag=RiverCheck,scores={game.node.node_state=1..2}] as @e[type=minecraft:armor_stand,tag=VarianceNode,tag=RiverCheck,sort=random,limit=1] run function exigence:variance/private/river_check_override
