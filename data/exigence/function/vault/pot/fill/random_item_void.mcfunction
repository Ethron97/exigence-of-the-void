...
execute if score toggle.vault debug matches 1 if score debug.level debug matches 3.. run say (D3) Random item void
...
# Call fill/from_item
execute as @e[x=537,y=0,z=472,dx=0,dy=0,dz=0,type=minecraft:item,tag=NVS] run function exigence:vault/pot/fill/from_item with entity @s Item

# Delete item with tag
kill @e[x=537,y=0,z=472,dx=0,dy=0,dz=0,type=minecraft:item,tag=NVS]
