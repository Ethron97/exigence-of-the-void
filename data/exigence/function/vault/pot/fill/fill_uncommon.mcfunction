...
execute if score toggle.vault debug matches 1 if score debug.level debug matches 4.. run say (D4 Vault) Fill (uncommon)
...
# If item, 20% chance to be rare
execute as @s[scores={Random=1,RandomB=..8}] run function exigence:vault/pot/fill/random_item_uncommon
execute as @s[scores={Random=1,RandomB=9..}] run function exigence:vault/pot/fill/random_item_rare
