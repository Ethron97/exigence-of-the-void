...
# Summon marker
function exigence:game/admin/new_marker

tag @e[type=minecraft:marker,tag=NewMarker] add PotMarker
$tag @e[type=minecraft:marker,tag=NewMarker] add $(vault_name)

execute if score toggle.vault debug matches 1 as @e[type=minecraft:marker,tag=NewMarker] at @s positioned ~-.5 ~ ~-.5 run function exigence:vault/debug/summon_pot_display
