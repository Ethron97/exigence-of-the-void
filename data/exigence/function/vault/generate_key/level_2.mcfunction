...
execute if score toggle.vault debug matches 1 if score debug.level debug matches 3.. run say (D3) Generate vault key (level 2)
...
execute as @e[x=-366,y=13,z=-106,dx=-118,dy=107,dz=-178,type=minecraft:marker,scores={node.property.object_level=2},tag=TreasureNode,tag=!KeyBlacklist\
