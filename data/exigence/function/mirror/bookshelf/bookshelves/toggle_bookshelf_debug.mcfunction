...
$execute if data storage exigence:debug {bookshelf:1} as @e[type=minecraft:armor_stand,scores={node.property.object_level=$(levels)},tag=BookshelfNode] run data merge entity @s {Glowing:1b,CustomNameVisible:1b,Invisible:0b,Marker:0b}
