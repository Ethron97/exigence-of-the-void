...
if score @s profile.node.profile_id = #compare profile.node.profile_id run scoreboard players set @s profile.shop_purchased.one_time_$(item_name) 1

# Coop profiles
$execute in exigence:profile_data positioned 8 128 8 as @e[distance=..140,type=marker,tag=ProfileNode] \
if score @s profile.player.coop_profile_id = #compare profile.node.profile_id run scoreboard players set @s profile.shop_purchased.one_time_$(item_name) 1

# Give advancement to player(s)
$advancement grant @s only exigence:unlockables/one_time/one_time_$(item_name)
$execute as @a if score @s profile.player.coop_profile_id = #compare profile.node.profile_id run advancement grant @s only exigence:unlockables/one_time/one_time_$(item_name)
