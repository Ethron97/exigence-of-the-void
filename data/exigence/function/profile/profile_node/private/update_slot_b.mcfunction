...
# Update solo sticker
execute if score #remaining_profiles Temp matches 1 at @s positioned ~ ~ ~ as @n[tag=ProfileTypeSticker,distance=..3] run data modify entity @s item.components."minecraft:custom_model_data".strings set value ["solo_selector"]
execute if score #remaining_profiles Temp matches 1 at @s positioned ~ ~ ~ as @n[tag=ProfileTypeSticker,distance=..3] run data modify entity @s transformation.scale set value [0.18f,0.18f,0.18f]