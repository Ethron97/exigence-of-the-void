...
execute if score #room_type Temp matches 10 in exigence:hub as @e[x=-98,y=0,z=798,dx=244,dy=256,dz=16,type=minecraft:marker,tag=ProfileSelectorNode] \
if score @s hub.entity.room_id = #room_id Temp at @s run function exigence:hub/profile_selector/load/load_chests

execute if score #room_type Temp matches 11 in exigence:hub positioned 0 100 0 as @e[type=minecraft:marker,tag=LockerRoomNode,distance=..200] \
