...
as @e[distance=..140,type=marker,tag=ProfileNode,scores={profile.node.coop_profile_id=1..}] \
...
execute if score @s profile.node.coop_profile_id matches 1.. as @e[distance=..300,type=marker,tag=ProfileNode,tag=WaitingForChests,sort=random] \
