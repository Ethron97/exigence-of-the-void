...
execute if score toggle.profile debug matches 1 if score debug.level debug matches 2.. run say (D2) Data to chest

execute if score #remove_tag Temp matches 1 run tag @n[tag=ProfileNode,distance=..0.1] remove ChestsLoaded
execute if score toggle.hub debug matches 1 if score debug.level debug matches 3.. if score #remove_tag Temp matches 1 run say (D3) REMOVING CHESTS LOADED TAG
