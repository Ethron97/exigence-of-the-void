...
# Load scores
$execute positioned 8 128 8 at @n[distance=..140,tag=ProfileNode,scores={profile.node.profile_id=$(profile_id)}] run function exigence:profile/profile_node/load/copy_profile_scores_to_player
...
$execute positioned 8 128 8 at @n[distance=..140,tag=ProfileNode,scores={profile.node.profile_id=$(profile_id)}] run function exigence:scoreboard/generated_functions/grant_advancements_from_scores
