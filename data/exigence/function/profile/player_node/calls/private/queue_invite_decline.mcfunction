...
execute if score toggle.hub debug matches 1 if score debug.level debug matches 3.. run say (D3 Hub) Queue invite decline
...
execute in exigence:hub as @e[x=-98,y=0,z=798,dx=244,dy=256,dz=16,type=minecraft:text_display,tag=StatusDisplay] if score @s hub.entity.player_id = #compare profile.node.player_id \
