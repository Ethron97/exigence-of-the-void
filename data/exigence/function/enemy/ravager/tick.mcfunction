...
execute if score seconds.cooldown tick_counter matches 15 run scoreboard players remove @s[scores={game.ravager.detect_thoughts=1..}] game.ravager.detect_thoughts 1
# Ambient particles around ravagers with any game.ravager.detect_thoughts amounts
execute if score @s game.ravager.detect_thoughts matches 1.. at @s run particle minecraft:electric_spark ~ ~0.5 ~ 1 1 1 0 1
...
execute if score @s game.ravager.eating_cookie matches 1 unless score @s game.enemy.stun_timer matches 1.. run data merge entity @s {NoAI:false}


# Ambient noise cooldown
execute if score @s game.entity.ambient_noise_cooldown matches ..0 at @s run execute if entity @a[tag=ActivePlayer,distance=14..24] run playsound minecraft:entity.ravager.ambient hostile @a[tag=ActivePlayer] ~ ~ ~ 1.3 0.8
execute if score @s game.entity.ambient_noise_cooldown matches ..0 store result score @s game.entity.ambient_noise_cooldown run random value 70..180
